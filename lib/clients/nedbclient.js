...
const operators = ['$lt', '$lte', '$gt', '$gte', '$in', '$nin', '$ne', '$exists', '$regex'];
const logicOps = ['$or', '$and', '$not', '$where'];
...
const createCollection = function(collectionName, url, options) {
    if (url === 'memory') {
        return new Datastore(Object.assign({}, {inMemoryOnly: true}, options));
    }
    let collectionPath = getCollectionPath(url, collectionName);
    return new Datastore(Object.assign({}, {filename: collectionPath, autoload: true}, options));
};
...
class NeDbClient extends DatabaseClient {
    ...
    findOneAndUpdate(collection, query, values, options) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            that.findOne(collection, query).then(function(data) {
                if (!data) {
                    if (options.upsert) {
                        return db.insert(Object.assign({}, values, _.omitBy(query,function(val,key) {
                            // first to omit key in ['$or', '$and', '$not', '$where']
                            // and then check whether it is a plain object
                            // in case it is a embedded object, check all keys in the obj and intersection with available operators
                            return _.indexOf(logicOps,key) > -1 || _.isPlainObject(val) && _(val).keys().intersection(operators).size() > 0; 
     ...
     * perpare collection
     * @param {String} collection Collection's name
     * @param {Object} options which will be used to create Datastore instance
     * @returns {Nedb/Datastore}
     */
    prepareCollection(collection, options) {
        return getCollection(collection, this._collections, this._path, options);
    }
