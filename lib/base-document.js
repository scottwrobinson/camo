...
class BaseDocument {
    ...
    validate() {
        ...
        _.keys(that._values).forEach(function(key) {
            ...
            if (that._schema[key].regex && isString(value) && !that._schema[key].regex.test(value)) {
                throw new Error('Value assigned to ' + that._meta.collection + '.' + key +
                    ' does not match the regex string ' + that._schema[key].regex.toString() + '. Value was ' + value);
            }
